...
**複数のプロジェクトを一度に表示できます。**
並べ方を「リソース別」に変えると、**同じリソースの作業が全プロジェクト横断で
...
**2026-09-04 にサーバ側の I/F 一覧を受領し、読み込み側は実物に合わせました。**
原文は [`docs/interface/received/`](docs/interface/received/) にあります。

分かった大きなこと:
- 新 GUI はサーバ側の `/external/` に置かれ、既存のトップ画面から iframe で開かれる
- 計画は **XML ではなく JSON** で受け取る（サーバ側が変換）。複数プロジェクトを 1 回で
- **保存・検証はサーバ側で未実装。** いまは読み込み専用（編集は画面内ドラフト）
- 「担当」は **BM3 の Skill 名**であって人の名前ではない
...
docs/
  ...
  ui/                     画面仕様
    ...
    16-adjust.md            調整パネル（1 つのリソースに特化して、試して・決めて・記録する）
